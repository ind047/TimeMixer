...
import re
import torch
# Commenting out sktime imports as we're not using them for P12
...
class Dataset_P12(Dataset):
    ...
    def __read_data__(self):
        ...
        # Store record_ids separately and remove from main data
        self.record_ids = df_raw['record_id'].values
        df_raw = df_raw.drop('record_id', axis=1)
        
        # Determine dataset splits based on dataset characteristics
        num_samples = len(df_raw)
        train_ratio, val_ratio = 0.7, 0.1  # 70% train, 10% val, 20% test
        
        border1s = [0, 
                    int(num_samples * train_ratio) - self.seq_len, 
                    int(num_samples * (train_ratio + val_ratio)) - self.seq_len]
        border2s = [int(num_samples * train_ratio), 
                    int(num_samples * (train_ratio + val_ratio)), 
                    num_samples]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
    ...
    def __getitem__(self, index):
        ...
        # Create dummy mark sequences (all zeros) since we don't have temporal features
